...
def tcg_save_url(record_id):
    """Save a TCGPlayer product URL to the record after validating the product ID."""
    record = ScanRecord.query.get_or_404(record_id)
    data   = request.get_json() or {}
    ...
    if not product_id:
        return jsonify({
            "status":  "error",
            "message": "That doesn't look like a TCGPlayer product URL. "
                       "Expected: tcgplayer.com/product/123456/...",
        }), 400

    canonical_url = f"https://www.tcgplayer.com/product/{product_id}"

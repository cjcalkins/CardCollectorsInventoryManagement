...
**Four of the six gate; two classify.** `check_html_escaping.py` and
`classify_innerhtml.py` are censuses — their exit `0` means *I classified these
files*, not *these files are clean*, and their deliverable is the table rather
than the status. That distinction is printed by both tools at runtime and was
still flattened into a column of green zeroes headed *gate green*, by someone
who had read the output. It is stated here because a person deciding whether to
run a tool reads this file, and a person deciding whether to merge reads the
status.
...
**Its numbers are not the XSS audit's numbers.** Today it reports **206 RAW and
86 escaped across 12 of 27 templates**, and the unit there is one `${…}`
**interpolation** reaching an `innerHTML` sink, counted over every template in
the tree. The audit counts something else: one `innerHTML` **assignment site**,
over the subset of files that have one. Different numerator *and* different
denominator — a single site carries many interpolations, and "every template in
the tree" is not the set "the templates with a site". Neither figure is derivable
from the other by any arithmetic anyone has produced, so there is no exchange
rate to publish and the two must not appear in the same sentence. Mixing them
that way has already produced a wrong count once.

**The audit's own figure is deliberately not quoted here, and that is the point
of the paragraph.** Every number in this file is regenerated by the tool that
prints it, so a stale one announces itself the next time anyone runs it. A figure
transcribed from a document outside this directory has no such mechanism: the one
that used to sit in this paragraph went stale within the hour of being written —
inside the sentence warning about exactly this — and nothing in the repo could
have noticed. Re-run the tool for this side of the contrast — and, as of this
commit, run `classify_innerhtml.py` and sum its `DYN` column for the other.
Both sides of the contrast are now regenerated by a tool in this directory, so
neither can go stale without saying so. Sum the per-file rows, not the `TOTAL`
line: that row is one field wider than the data rows and a field index that is
right for the data is off by one on it, which has produced two plausible wrong
sums.
...
### `classify_innerhtml.py` — the innerHTML census the audit sizes itself with

    tools/classify_innerhtml.py                    # templates/ in this checkout
    tools/classify_innerhtml.py path/to/templates
    tools/classify_innerhtml.py --json /tmp/inv.json   # line numbers, opt-in

**A census, not a gate. Exit 0 means an inventory was emitted.** The `DYN`
column — `interp` + `ident` + `expr` — is the review surface, and it is where
the XSS audit's "how much is left" comes from. `static` is stated in the
docstring as the mechanism rather than as a paraphrase — *strip every quoted
chunk and what remains is whitespace and `+`, nothing else* — because two
successive paraphrases of it over-promised, the second one in review here.
`ident` means a bare identifier was assigned, so interpolation may have
happened upstream, which is why a grep for `${` inside an `innerHTML`
assignment under-counts.

Control: `651582a58ed5…:templates/inventory_detail.html` must reproduce the
hand-verified `82/4/78` — `static 31`, `dynamic 47` (`interp 39`, `ident 5`,
`expr 3`) — **and both line lists member for member.** Counts and lists are
asserted together because on this file neither alone is sufficient, which is a
measurement rather than belt-and-braces: `:2599` and `:2604` move
`static → interp` and neither of those buckets has a line list, while `:2221`
and `:1734` move *within* `dynamic`, leaving `expr` at 3 before and 3 after
with a different member in it.

**The expectation is hand-written and its input is pinned, and both halves of
that matter.** `BASELINE` is a hand-counted decomposition; deriving it by
running this same classifier would make the self-test `classifier(x) ==
classifier(x)`, which cannot fail and so cannot detect a regression. Reading it
against the *working tree* is the opposite defect — this tool's control file is
also the audit's largest remaining target, so every legitimate edit to it used
to fail the self-test with a message accusing the classifier, and the repair
that unsticks that is editing `BASELINE` to match, which makes the control a
copy of the subject. Demonstrated both ways at the commit that added this
section:

    the FILE changes (a data-bearing sink appended to the working tree's copy)
      before   SELF-TEST FAILED, "classifier does not reproduce…"   rc 2
      after    self-test OK; the row moves 47 -> 48                 rc 0
    the CLASSIFIER changes (tree untouched, mutation asserted applied first)
      classify(): ident -> expr, one token
               SELF-TEST FAILED  ident 5->0, expr 3->8, both lists  rc 2
      the continuation rule reverted to accumulate-while-unbalanced
               SELF-TEST FAILED  static 31->33, dynamic 47->45,
                                 expr_lines [2221,…] -> [1734,…]    rc 2

Pinning the input removes the spurious failure, so nothing tempts the circular
edit; changing what the numbers *should* be now means moving `CONTROL_REF`,
which is visible in a diff. **The first of those two mutants passed a
counts-only baseline** — it empties the `ident` bucket, and the JSON
deliverable with it, without moving `dynamic` — which is the other half of why
the lists are asserted.

**The JSON write is opt-in.** A default write would either depend on the cwd —
which is how the pre-repo version printed a correct table and then died `rc 1`
from any directory but one — or leave an untracked file in the clone you ran it
in. `.scratch/` is not in this repo's `.gitignore`, so a repo-relative default
puts `?? .scratch/` in `git status` on a fresh clone. `--json PATH` writes
where the caller says and prints the absolute path it used.

Does not see: sinks other than `innerHTML` — `insertAdjacentHTML` occurs 3
times in `templates/` and is invisible here, while `outerHTML =` and
`document.write` are 0 today, so the gap is small now and will not announce
itself if that changes. Computed access (`el["inner"+"HTML"]`) is not matched,
0 occurrences today. It does not **resolve intermediates**: `ident` says data
may have been assembled elsewhere, never whether that assembly escaped. Its
continuation accumulator stops after 40 lines. And it does not evaluate Jinja —
includes, macros and `{% %}` branches are text, so a sink in a branch that never
renders is still counted.

Two limitations err **into** the review surface, and the direction is worth
stating each time because a limitation whose direction is unstated reads as a
hole. The chunk stripper is escape-blind (`'[^']*'` ends inside `'it\'s'`), so
a literal containing an escaped quote falls out of `static` into `expr`; there
are 0 backslashes in all 78 write expressions at the control, so it does not
touch the pinned numbers. And a ternary over two literals selected by a boolean
is `expr` — getting that right means knowing the selector is a boolean, which
is a type checker, and this is a line-oriented scanner. Both cost a reader a
minute; neither can hide a write.
